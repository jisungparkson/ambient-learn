...
serve(async (req) => {
  ...
  console.log(`[요청 ${requestId}] Edge Function 시작:`, {
    method: req.method,
    url: req.url,
  });
  ...
  try {
    ...
    try {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error:
            "요청 데이터를 파싱할 수 없습니다. 올바른 JSON 형식인지 확인해주세요.",
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    console.log(`[요청 ${requestId}] 문서 생성 요청:`, {
      documentType,
      title,
    ...
    if (!documentType || !title || !content) {
      return new Response(
        JSON.stringify({
          success: false,
          error:
            "필수 입력값이 누락되었습니다. documentType, title, content는 모두 필수입니다.",
          missingFields: {
    ...
    const createGiamunPrompt = (
      ...
      const EXAMPLES_DATA = {
        공문: {
          ...
          structure:
            "제목, 담당부서, 기안자, 기안일자, 목적, 배경, 내용, 일정, 기대효과",
        },
        가정통신문: {
        ...
        기안문: {
          ...
          structure:
            "제목, 담당부서, 기안자, 기안일자, 목적, 배경, 내용, 예산내역, 일정, 기대효과",
        },
      };

      const docInfo =
        EXAMPLES_DATA[documentType as keyof typeof EXAMPLES_DATA] ||
        EXAMPLES_DATA["기안문"];

      // 다양한 문체와 스타일을 위한 랜덤 요소
      const writingStyles = [
        "공식적이고 격식있는 문체",
        "친근하면서도 정중한 문체",
        "간결하고 명확한 문체",
...
**중요 지침:**
- ${randomStyle}로 작성해주세요
- ${randomFocus}을 강조하여 작성해주세요
- 사용자가 제공한 제목과 내용을 바탕으로 구체적이고 실질적인 내용을 작성해주세요
- 매번 다른 표현과 구조를 사용하여 다양성을 보여주세요
- 교육기관의 실제 상황에 맞는 현실적인 내용으로 작성해주세요
- 기안문의 목적과 배경을 명확히 구분하여 작성해주세요
- 현재 날짜를 기준으로 적절한 기안일자를 설정해주세요

다음 JSON 형식으로 정확히 응답해주세요:
{
  "title": "문서 제목 (사용자 입력을 반영한 구체적인 제목)",
  "department": "담당 부서 (교육기관에 적합한 부서명)",
  "drafter": "기안자 (적절한 직책과 이름)",
  "date": "기안일자 (YYYY-MM-DD 형식)",
  "purpose": "기안 목적 (사용자 입력을 바탕으로 한 구체적인 목적)",
  "background": "추진 배경 (현실적이고 설득력 있는 배경 설명)",
  "content": "주요 내용 (사용자 입력을 상세히 반영한 구체적인 내용)",
  "budget": "예산 내역 (해당 시에만, 없으면 '해당 없음')",
  "schedule": "추진 일정 (실현 가능한 구체적인 일정)",
  "expected_effect": "기대 효과 (측정 가능하고 구체적인 효과)",
  "appendix": "첨부 자료 목록 (필요 시)",
  "contact": "담당자 연락처 정보"
}

모든 내용은 한국어로 작성하고, 사용자의 요구사항을 충실히 반영하여 매번 다른 스타일로 작성해주세요. JSON 형식을 정확히 지켜주세요.`;
    };
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a creative and experienced Korean educational document writer specializing in K-EDUFINE format. You excel at creating diverse, contextually appropriate documents that vary in style, tone, and structure while maintaining professional standards. Always respond with valid JSON format and ensure each document is unique and tailored to the specific requirements.",
            },
    ...
    // 예상치 못한 오류 처리
    console.error(
      `[요청 ${requestId}] Unexpected error in generate-document function:`,
      error
    );
